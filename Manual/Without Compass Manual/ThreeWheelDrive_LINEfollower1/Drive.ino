...
void startMotion(wheel ** whee){
  for(int i=0;i<3;i++)
  {
    int pwmval = (float)whee[i]->rpm * 255/(float)rpmmax;
    //Serial.println(pwm);
    if(pwmval > 0){
      ...
      analogWrite(whee[i]->pinpwm, abs((int)pwmval)/PWMfactor);
  }
  Serial.println();
}
...
void calcRPM(int omega, int angle, int transvel, wheel **whee){
  int r = 1;
 whee[0]->trans_rpm = -0.5584*transvel*2.1*cos(degtorad(angle)); //front wheel
